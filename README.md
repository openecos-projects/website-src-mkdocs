...
- Edit
  ...
  If you need to add new images or videos in Markdown files, please follow this workflow: first, upload lossless original images (JPG or PNG) or pre-compressed videos (MP4) to the corresponding directory under `web/res/img`. Then run `make gen-webp` in the `web` directory, and scripts in `web/tpl` will automatically convert original images to smaller WEBP files. Finally, reference the generated WEBP images in your Markdown files.
